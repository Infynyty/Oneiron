...
public class PlayerDamageListener implements Listener {
    ...
    public void onPlayerDamaged(EntityDamageByEntityEvent e) {
        if(JoinListener.getAllOneironPlayers().containsKey(e.getEntity().getUniqueId())) {
            if(!e.getDamager().hasMetadata(Util.ID)) return;
            e.setDamage(0);
            OneironPlayer oneironPlayer = JoinListener.getAllOneironPlayers().get(e.getEntity().getUniqueId());
